...
function pagination(value) {
  ...
  pacientes = array.slice(offset, offset + limit);
  ...
  for (let i = 0; i < paginationPrint.pages; i++) {
    itemPagination += `<button class="btn btn-outline-secondary " style="margin-left: 5px; width: 50px"  onclick="button(${
      i + 1
    })">${i + 1}</button>`;
  }
...
function preencherTabela(guias) {
  ...
  guias.forEach((guide) => {
    ...
    html += `
            <tr>
            <td><input ${
              checado[guide.number] ? "checked" : "unchecked"
            } onchange="selecionarCheck('${
      guide.number
    }')" type="checkbox" id="check"></td>
...
const filtrarTabela = () => {
  ...
  const guiasFiltradas = dados.guides.filter((guide) => {
    ...
    const dataPacientes = new Date(guide.start_date).toISOString().slice(0, 10);
    ...
    if (
      dataInicial &&
      dataInicial !== dataPacientes &&
      dataInicial > dataPacientes
    ) {
      valido = false;
    }
    if (dataFinal && dataFinal !== dataPacientes && dataFinal < dataPacientes) {
      valido = false;
    }
